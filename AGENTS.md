...
## Scope
Work only on **LectureAI** as a local-first lecture recording, transcription, notes, study, export, and public-web app. Do not introduce or reference unrelated products/projects. Preserve the stable production app unless a change is explicitly intended for release. The active integration branch is `expo-unified-lectureai`; PR #28 must stay draft until the physical iPhone + iPad acceptance gate passes.
...
2. A saved recording is not considered verified merely because a file exists. Keep beginning/middle/end playback checks plus explicit user confirmation and recovery warnings.
...
8. `[uncertain]`, `[inaudible]`, or segments explicitly marked uncertain must not be promoted into trusted notes/study facts.
...
10. SDK 57 recording should use document storage and unexpected recorder stops must be preserved/reported rather than silently treated as an active session.

## Optional LectureAI cloud features
LectureAI remains local-first. If a LectureAI-specific cloud backend is added later:
- keep local recording + local library as the default;
- raw lecture audio must never upload silently;
- use LectureAI-specific tables/schema only;
- enforce per-user ownership/RLS for user data;
- never ship service-role credentials to web/Expo clients;
- keep cloud sync optional and clearly separate from the protected local original.
...
- explicit beginning/middle/end playback verification;
...
- interruption/unexpected-stop recovery test where practical;
