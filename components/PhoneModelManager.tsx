...
export function PhoneModelManager({ installed, onInstalled }: { installed: boolean; onInstalled: () => void }) {
  ...
  const [message, setMessage] = useState(installed ? 'A multilingual Whisper model was prepared on this device.' : 'Prepare the phone/iPad model before class so transcription does not need to download it afterward.');
  ...
  return <section className="settings-card wide-card">
    <span className="soft-icon">{installed || progress === 100 ? <Check size={20} /> : supported ? <Cpu size={20} /> : <TriangleAlert size={20} />}</span>
    ...
    <button className="secondary-button" onClick={() => void prepare()} disabled={busy || !supported}>
      <Download size={16} /> {busy ? 'Preparing model…' : installed ? 'Verify / refresh model cache' : 'Download model before class'}
    </button>
