...
export async function initializeDatabase() {
  ...
  for (const status of ['preparing', 'transcribing', 'checking', 'generating-notes'] as const) {
    ...
    await Promise.all(stuck.map(async (lecture) => {
      if (!lecture.segments.length && lecture.windowsTranscriptionJob?.id) return db.put('lectures', { ...lecture, status: 'interrupted', statusMessage: `Windows transcription interrupted at ${Math.round(lecture.windowsTranscriptionJob.completedAudioSeconds || 0)} / ${Math.round(lecture.windowsTranscriptionJob.totalAudioSeconds || 0)} seconds · use Check / Resume; do not upload again`, processingProgress: lecture.windowsTranscriptionJob.progress, updatedAt: new Date().toISOString() });
      if (!lecture.segments.length && lecture.phoneTranscriptionCheckpoint) return db.put('lectures', { ...lecture, status: 'interrupted', statusMessage: `Browser transcription interrupted at ${Math.round(lecture.phoneTranscriptionCheckpoint.completedAudioSeconds || 0)} / ${Math.round(lecture.phoneTranscriptionCheckpoint.totalAudioSeconds || 0)} seconds · completed windows are saved`, processingProgress: undefined, updatedAt: new Date().toISOString() });
